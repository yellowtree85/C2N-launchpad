...
contract AllocationStaking is OwnableUpgradeable {
    ...
    function updatePool(uint256 _pid) public {
        ...
        // bug:lastTimestamp <= pool.lastRewardTimestamp the reward had been distributed indicates that the time is wrong should return
        if (lastTimestamp <= pool.lastRewardTimestamp) {
            lastTimestamp = pool.lastRewardTimestamp;
        }
        // bug: lastTimestamp <= pool.lastRewardTimestamp The following codes does not make sense
