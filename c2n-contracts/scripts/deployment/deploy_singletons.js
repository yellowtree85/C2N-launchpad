...
async function main() {
  ...
  const admin = await Admin.deploy(c.admins); //0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266
  ...
  console.log(
    "AllocationStakingProxy Admin address is : ",
    proxyAdminContract.address
  );
  ...
  console.log(
    `salesFactory.setAllocationStaking ${allocationStaking.address} done.`
  );

  const totalRewards = ethers.utils.parseEther(
  ...
  await tx.wait();
  console.log(
    `token.approve(${allocationStaking.address}, ${totalRewards.toString()});`
  );
