...
export function listenToWallet() {
  ...
  async function handleChainChanged(chainId: number) {
    ...
    if (!chain) {
      // select 31337, this would switch to 31337
      return;
    }
...
export const useWallet = () => {
  ...
  async function getAccount(options?) {
    ...
    if (!checkMetaMask()) {
      ...
      dispatch(walletActions.setisWalletInstalled(false)); //把钱包未安装的状态更新到 Redux 全局状态中。
    ...
    if (!window.ethereum.isConnected()) {
      ...
      return Promise.reject(); //返回一个 rejected 状态的 Promise（相当于抛出一个错误），用于阻止继续执行后续的钱包连接逻辑。
    }
    try {
      const accounts = await window.ethereum.request({
        method: "eth_requestAccounts",
      });
      dispatch(contractActions.setWalletAddress(accounts[0])); //setWalletAddress作用是把钱包地址保存到全局状态 Redux 的 dispatch 方法，用于触发状态变更
