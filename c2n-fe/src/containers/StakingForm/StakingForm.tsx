...
function TierCard(props: any) {
  ...
  return (
    <div className={[styles["tier-card"], "tier-card"].join(" ")}>
...
export default function StakingForm(props: StakingFormProps) {
  ...
  async function onStakeButtonClick() {
    ...
    console.log(
      "depositNum",
      parseEther(depositNum).toString(),
      "wei",
      " balance",
      balance.toString(),
      "wei"
    );
    if (parseEther(depositNum) > balance) {
      setErrorMessage(
        `Not enough ${depositSymbol} to stake! balance: ${balance.toString()} wei`
      );
      return;
    }
  ...
  function onWithdrawButtonClick() {
    console.log(
      "withdrawNum",
      parseEther(withdrawNum).toString(),
      "wei",
      " depositedAmount",
      depositedAmount.toString(),
      "wei"
    );
    if (parseEther(withdrawNum) > depositedAmount) {
      setErrorMessage(
        `Not enough ${depositSymbol} to withdraw! depositedAmount: ${depositedAmount.toString()} wei`
      );
      return;
    }
        ...
        <section className={styles["stake-token"]}>
          <Row justify="space-between" gutter={[16, 16]}>
              ...
              <Row justify="space-between" gutter={[16, 16]}>
                <Col span={isDesktopOrLaptop ? 12 : 24}>
                  <Row justify="space-between">
                    ...
                    <div
                      className="max"
                      onClick={() => setDepositNum(formatEther(balance))}
                    >
                      MAX
                    </div>
                  </Row>
                ...
                <Col span={isDesktopOrLaptop ? 12 : 24}>
                  <Row justify="space-between">
                    ...
                    <div
                      className="max"
                      onClick={() =>
                        setWithdrawNum(formatEther(depositedAmount, 4))
                      }
                    >
                      MAX
                    </div>
            ...
            <Col className="right" span={isDesktopOrLaptop ? 12 : 24}>
              ...
              <LeftOutlined
                onClick={() => {
                  tiers.current && tiers.current.prev();
                }}
                className={styles["tier-left-arrow"]}
              />
              <RightOutlined
                onClick={() => {
                  tiers.current && tiers.current.next();
                }}
            ...
            <Col span={isDesktopOrLaptop ? 8 : 24}>
              <Row className={styles["box"]} align="middle" gutter={4}>
                ...
                <Col span={24} className={styles["value"]}>
                  <Motion
                    defaultStyle={{ x: 0 }}
                    style={{ x: spring(depositedAmountInEther) }}
                  >
                    {(value) => (
                      <>
                        {seperateNumWithComma(value.x.toFixed(2))}{" "}
                        {depositSymbol}
                      </>
                    )}
                  </Motion>
                  <div className={styles["extra"]}>
                    ~$
                    {seperateNumWithComma(
                      (depositedAmountInEther * stakedTokenToUsd).toFixed(2)
                    )}
                  </div>
                </Col>
              </Row>
            ...
            <Col span={isDesktopOrLaptop ? 8 : 24}>
              <Row className={styles["box"]} align="middle" gutter={4}>
                ...
                <Col span={24} className={styles["value"]}>
                  $
                  <Motion
                    defaultStyle={{ x: totalDepositsInUsd }}
                    style={{ x: spring(totalDepositsInUsd) }}
                  >
