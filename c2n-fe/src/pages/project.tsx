...
export default function Pool({ Component, pageProps }: AppProps) {
  ...
  useEffect(() => {
    ...
    if (projectInfo) {
      ...
      setPaymentTokenAddress("0x1Dd5dcB05E451EfC6d6D4Fb2B905b02Dc3679aB4"); //C2NToken
    }
  ...
  function getParticipateAmount() {
    ...
    const f = new FormData();
    Object.entries({ accountId: walletAddress, productId: pId }).forEach(
      ([key, value]) => {
        f.append(key, value as string);
      }
    );
  ...
  async function participate(value) {
    ...
    const paymentAmount = BigNumber.from(projectInfo.tokenPriceInPT).mul(
      ~~value.value
    );
    /**
     * 购买代币
    ...
    <div className={styles["token-info"] || ""}>
      ...
      <Row className={styles["row"]}>
        ...
        <Col span={isDesktopOrLaptop ? 12 : 24} className={styles["value"]}>
          {projectInfo &&
            seperateNumWithComma(
              formatEther(projectInfo.amountOfTokensToSell) / 1.25
            )}
        </Col>
      </Row>
      <Row className={styles["row"]}>
  ...
  const getActionButton = () => {
    ...
    if (statesReady && [1].includes(status)) {
      if (!isRegistered && !canRegister) {
        return (
          <AppPopover
            content={"You have participated another project."}
            wrap={true}
          >
            <TransactionButton
              disabled={true}
              ...
              className={
                "button " + styles["register-button"] + " " + "disabled"
              }
            >
              {
                // isUserRegister ? 'Participate' :
  ...
  function getReferralCode() {
    ...
    return axios.post("/boba/referral/code", f).then((res) => {
  ...
  const referralLink = useMemo(() => {
    if (typeof window !== "undefined") {
      return (
        window.location.href.replace(/aff=.*/, "") + `&aff=${referralCode}`
      );
    } else {
      return "";
    }
  ...
  return (
    <main
      ...
      style={
        trickers
          ? {
              backgroundImage: `url(${trickers?.background})`,
              backgroundSize: "100%",
